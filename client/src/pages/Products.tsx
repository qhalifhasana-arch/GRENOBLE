    ...
    <div className="min-h-screen bg-gray-50 pb-28" data-testid="products-page">
      <div className="bg-gradient-to-br from-green-700 to-emerald-800 px-5 pt-12 pb-8 rounded-b-[2rem]">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-green-200/60 text-sm mt-1">Investissez dans le futur durable</p>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          products?.map((product) => (
            <Card key={product.id} className="border-0 shadow-sm overflow-hidden rounded-2xl bg-white" data-testid={`card-product-${product.id}`}>
              <div className="h-32 bg-gradient-to-br from-green-600 to-emerald-700 relative p-5 flex flex-col justify-between overflow-hidden">
                <div className="absolute -right-4 -bottom-4 opacity-10">
                  <TrendingUp className="w-24 h-24 text-white" />
                </div>
                <div className="flex justify-between items-start relative z-10">
                  <Badge className="bg-white/20 hover:bg-white/30 text-white border-0 backdrop-blur px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider">
                    Pack VIP {product.vipLevel}
                  </Badge>
                  <div className="text-white text-right">
                    <p className="text-xs uppercase font-bold tracking-wider opacity-60">Retour</p>
                    <p className="font-extrabold text-xl tracking-tight">{product.totalReturn.toLocaleString()} <span className="text-xs">F</span></p>
                  </div>
                </div>
                <h3 className="text-xl font-extrabold text-white relative z-10">{product.name}</h3>
              </div>

              <CardContent className="p-6 grid grid-cols-2 gap-4 bg-white">
